...
```
singularity build SAW_v2.1.1.sif docker://stomics/saw:02.1.1 
```
...
usage: sh stereoRun.sh -m maskFile -1 read1 -2 read2 -g indexedGenome -a annotationFile -o outDir -i image -t threads -s visualSif
    ...
    -p formatted string for giving needed parameters to semiautoregister, not requested
```
...
sh stereoRun.sh \
    ...
    -o outDir \
    -i image_dir_path
when tissue image was given and parameters for re-registration was given
sh stereoRun.sh \
    -m SN.h5 \
    -1 lane_read_1.fq.gz \
    -2 lane_read_2.fq.gz \
